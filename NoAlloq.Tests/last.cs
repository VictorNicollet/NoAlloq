...
namespace NoAlloq.Tests
{
    public sealed class last
    {
        ...
        {
            Span<int> values = stackalloc int[4] { 0, 1, 2, 3 };
            Assert.Equal(3, values.Where(new IsOdd()).Box().Last());
        }
        ...
        public void where_is_odd_none_boxed()
        {
            ...
            try
            {
                var found = values.Where(new IsOdd()).Box().Last();
                Assert.True(false);
            }
            catch (InvalidOperationException e)
            {
        ...
        {
            Span<int> values = stackalloc int[4] { 1, 2, 3, 4 };
            Assert.Equal(4, values.Box().Last());
        }
        ...
        {
            Span<int> values = stackalloc int[4] { 0, 1, 2, 3 };
            Assert.Equal(3, values.Box().Last(v => v % 2 != 0));
        }
        ...
        {
            Span<int> values = stackalloc int[4] { 1, 2, 3, 4 };
            Assert.Equal(5, values.Box().Select(v => v + 1).Last(v => v % 2 != 0));
        }
